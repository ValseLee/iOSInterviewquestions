...
REST API에서 Method들의 차이점: 주요 HTTP 메서드로는 GET(자원 조회), POST(자원 생성), PUT(자원 갱신), DELETE(자원 삭제) 등이 있으며, 각각의 메서드는 RESTful 서비스에서 다른 작업을 수행합니다.

HTTP 상태 코드: HTTP 상태 코드는 서버 응답의 상태를 나타내는 코드로, 200번대는 성공, 300번대는 리다이렉션, 400번대는 클라이언트 오류, 500번대는 서버 오류를 의미합니다.
